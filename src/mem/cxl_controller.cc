...
CXLcontroller::CXLcontroller(const CXLcontrollerParams &params)
    : SimObject(params), cpu_port(params.name + ".cpu_port", this),
      mem_port(params.name + ".mem_port", this),
      device_addr_ranges(params.device_ranges),
      cxl_redirect_strategy(params.cxl_strategy), frag_perc(params.frag_perc),
      frag_seed(params.frag_seed), latency(params.cxl_latency),
      req_waiting_retry(false), resp_waiting_retry(false),
      ...
      speed_device_idx(0)  // The index of the fastest device (0 by default)
{
    ...
    if (cxl_redirect_strategy == "direct") {
        ...
        // All zero means unmapped (hence entries are biased by one)
        size_t num_blocks = total_size / BLOCK_SIZE;
        addr_map = static_cast<Addr *>(calloc(num_blocks, sizeof(Addr)));
        assert(addr_map != nullptr); // Ensure successfull allocation
    }
}
...
CXLcontroller::~CXLcontroller() {
    ...
    // Release all queued packets (if any)
    for (auto &entry : req_queue) {
        PacketPtr pkt = entry.second;
        auto got = reverse_addr_map.find(pkt);
        if (got != reverse_addr_map.end()) {
            delete got->second;
    ...
    // Free packets in reverse map (if any)
    for (auto &entry : reverse_addr_map) {
        delete entry.second;
    }
    reverse_addr_map.clear();

    // Free the queue (if any)
    for (auto &entry : resp_queue) {
        delete entry.second;
    }
}
...
void CXLcontroller::initFragMap(Addr total_size) {
    // Sanity check
...
Port &CXLcontroller::getPort(const std::string &if_name, PortID idx) {
...
Addr CXLcontroller::physToMapIndex(Addr phys_addr, Addr granularity) const {
    Addr taken_space = 0;
    for (int i = 0; i < device_addr_ranges.size(); i++) {
...
Addr CXLcontroller::mapIndexToPhys(Addr map_idx, Addr granularity) const {
...
PacketPtr CXLcontroller::remapPacket(PacketPtr pkt, Addr new_addr,
                                     bool isTiming) {
    ...
    // Without this the device sees an uninitialized buffer
    if (pkt->isWrite()) {
        newPkt->setData(pkt->getConstPtr<uint8_t>());
    }

    if (pkt->needsResponse()) {
        // Park the original until the device responds; the response path
        // looks it up, copies the data back and frees the device packet.
        reverse_addr_map[newPkt] = pkt; // Device pkt -> Original pkt
    } else {
        // WritebackDirty, CleanEvict and PrintReq don't need Response
        // Free them
        delete pkt;
    }
...
void CXLcontroller::updateOldPacket(PacketPtr oldPkt, PacketPtr newPkt) {
    // Make response
...
PacketPtr CXLcontroller::handleDirect(PacketPtr pkt, bool from_cpu,
                                      bool isTiming) {
...
PacketPtr CXLcontroller::handleRandom(PacketPtr pkt, bool from_cpu,
                                      bool isTiming) {
    if (from_cpu) {
        // Received a request packet from cpu side
        ...
        if (!isMapped(addr_map_idx)) {
            ...
            setMapped(addr_map_idx, next_block);
            ...
            return remapPacket(pkt, getMapped(addr_map_idx) + offset, isTiming);
        }
    } else {
        // Received a response packet from memory side
...
PacketPtr CXLcontroller::handleSpeed(PacketPtr pkt, bool from_cpu,
                                     bool isTiming) {
    if (from_cpu) {
        // Received a request packet from cpu side
        assert(pkt->getSize() <= BLOCK_SIZE);
...
void CXLcontroller::trySendResp() {
...
Tick CXLcontroller::handleRequest(PacketPtr pkt, std::string req_type) {
...
bool CXLcontroller::handleResponse(PacketPtr pkt) {
    PacketPtr newPkt;
    if (cxl_redirect_strategy == "direct") {
...
///// CpuSidePort /////
///////////////////////
AddrRangeList CXLcontroller::CpuSidePort::getAddrRanges() const {
    return owner->mem_port.getAddrRanges();
}

void CXLcontroller::CpuSidePort::recvFunctional(PacketPtr pkt) {
    owner->handleRequest(pkt, "functional");
}

Tick CXLcontroller::CpuSidePort::recvAtomic(PacketPtr pkt) {
    return owner->handleRequest(pkt, "atomic");
}

bool CXLcontroller::CpuSidePort::recvTimingReq(PacketPtr pkt) {
    owner->handleRequest(pkt, "timing");
    return true; // Never blocks (unbounded queue)
}

void CXLcontroller::CpuSidePort::recvRespRetry() {
...
///// MemSidePort /////
bool CXLcontroller::MemSidePort::recvTimingResp(PacketPtr pkt) {
    assert(owner->handleResponse(pkt));
    return true;
}

void CXLcontroller::MemSidePort::recvReqRetry() {
    panic_if(!owner->req_waiting_retry,
             "Should never receive retry without a blocked packet");
    owner->trySendReq();
}

void CXLcontroller::MemSidePort::recvRangeChange() {
